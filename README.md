...
1. Set up [Configuration File](#configuration-file)
2. Set version number in [Version File](#version-file)
...
The scripts must be run a specific order to ensure all inputs for a given script have already by generated by previous scripts. The order of the scripts is listed in the `run-model.sh` bash script.

In theory, one could execute this bash script to run all the R scripts sequentially after setting up the configuration file.
...
- `SHEDS_BTO_ROOT`: within the `config.sh` file (see below), this variable should be set to a local path that serves as the root directory for all model versions (e.g. `/path/to/bto-model-data`)
- `SHEDS_BTO_VERSION`: within the `version.sh` file, this variable should be set to a unique model version, which can be any string although typically will be of the form `X.Y` where `X` is the major version number, and `Y` is the minor version number (e.g. `1.0`).
...
The version can be set to any string, because it is simply used to generate the model working directory (see above). During development, for example, the version could be set to `1.0-dev`.
